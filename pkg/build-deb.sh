...
prepare() {
  ...
  local src="$BUILD_DIR/pgedge-safesession-${SAFESESSION_VERSION}"

  echo "Moving Debian packaging into source directory..."
  cp -rp "${COMPONENT_DIR}/deb/debian" "${src}/"
  cp "${src}/debian/control.in" "${src}/debian/control"
  sed -i "s|PG_MAJOR_VERSION|${PG_MAJOR_VERSION}|g" "${src}/debian/control"
  mv "${src}/debian/pgedge-postgresql-safesession.install" \
     "${src}/debian/pgedge-postgresql-${PG_MAJOR_VERSION}-safesession.install"
...
build() {
  ...
  cd "$BUILD_DIR/pgedge-safesession-${SAFESESSION_VERSION}"
  echo "Building Debian package..."
  DISTRO=$(lsb_release -cs)
  # SAFESESSION_DEB_VERSION carries the '~<pretag>' form for pre-releases so
  # they sort below stable; it equals SAFESESSION_VERSION for a GA build.
  ...
  {
      echo "pgedge-safesession (${SAFESESSION_DEB_VERSION}-${SAFESESSION_BUILDNUM}.${DISTRO}) ${DISTRO}; urgency=low"
      echo ""
      echo "  * Update Release."
      echo ""
